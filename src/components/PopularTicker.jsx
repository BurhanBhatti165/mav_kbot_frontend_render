...
export default function PopularTicker() {
  ...
  return (
    <div className="fixed h-8 bottom-0 left-0 right-0 bg-[#111] border-t border-gray-800 py-2 px-3 shadow-lg flex items-center gap-6 overflow-hidden">
      ...
      <div className="relative overflow-hidden w-full bg-black py-2">
        {
        ...
        <div className="animate-marquee whitespace-nowrap">
          ...
          {coins.map((coin, index) => (
            <div
              key={index}
              className="inline-flex items-center gap-2 bg-[#1e1e1e] px-3 py-1 mx-2 rounded-lg hover:bg-[#2a2a2a] transition"
            >
              <span className="font-semibold text-green-400">{coin.symbol}</span>
              <span className="text-gray-200">${coin.price.toLocaleString()}</span>
            </div>
          ))}
